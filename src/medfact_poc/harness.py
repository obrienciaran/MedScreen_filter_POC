...
Key measurement decisions:
  * **Retrieval recall is ground-truth anchored and stance-independent**: did the
    pool contain a gold answer-key PMID? This is the most trustworthy number because
    it does not depend on the (LLM) stance judge being correct.
  * **Stance recall is conditional**: given an answer-key doc was retrieved, did the
    stance step recognize it as refuting? Separating these localizes failures.
...
from .retrieval import links, semantic
from .schema import (
    Candidate,
    ClaimReport,
    ClaimStatus,
...
def retrieve_pool(
    ...
    *,
    ...
    """Build the candidate pool for a claim from every source + link expansion.

    When ``use_cache`` and a cached pool exists, read it from the store (offline);
...
def embed_pool(
    ...
    """Embed the claim + pool, return pool ranked by cosine to the claim."""
