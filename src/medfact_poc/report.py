"""Render harness results to a markdown report + a per-claim CSV."""
...
def _render_md(
    ...
    lines = [
        ...
        f"| Stance recall (overall) | {pct(m.stance_recall_overall)} | retrieval × stance combined |",
    ...
    lines += [
        ...
        "  refutation isn't even retrieved, no scoring layer can use it.",
        "- A large gap between retrieval recall and stance recall means the bottleneck is the",
        "  stance judge, not retrieval — different fix.",
        "- `not_indexed` / `entity_miss` are retrieval failures; `retrieved_not_recognized` /",
        "  `condition_mismatch` / `tier_inversion` are stance/aggregation failures.",
        "- With a stub stance/embedder backend these numbers are placeholders for plumbing",
