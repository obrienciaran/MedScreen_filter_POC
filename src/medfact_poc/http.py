"""Shared HTTP client + config for evidence sources.

Centralizes rate limiting and TLS handling. Some environments (corporate proxies,
sandboxes) terminate TLS with a self-signed cert that breaks default verification;
set ``MEDFACT_INSECURE_TLS=1`` or ``MEDFACT_CA_BUNDLE=/path/to/ca.pem`` to cope without
editing code. NCBI asks callers to identify themselves (``NCBI_EMAIL``) and an
``NCBI_API_KEY`` raises the rate limit from 3 to 10 requests/sec.
"""
...
def ncbi_params(extra: dict[str, str]) -> dict[str, str]:
    """Augment E-utilities params with identification/key when available."""
    params = dict(extra)
    if NCBI_EMAIL:
