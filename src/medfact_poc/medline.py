...
Both the harness source (`sources/pubmed.py`, which builds Candidates from a live efetch
call) and the filter ingester (`ingest.py`, which builds PaperRecords from lakehouse files)
parse the same ``PubmedArticle`` schema. These pure element helpers are the shared leaves,
kept in one place so the two parsers cannot drift. Each parser still owns how it assembles
its own model and which relationships it keeps.
"""
