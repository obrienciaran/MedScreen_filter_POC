...
shows the verdict (refutes, supports, neutral). A study known to disprove a claim is ringed
in gold when the search found it, and shown as a dashed "not retrieved" link to an orange
triangle when the search missed it, so recall gaps are visible at a glance. Retraction links
between papers are drawn as well.
        ...
        for pmid in refuting:
            ...
            if ev_id not in nodes:
                nodes[ev_id] = GraphNode(
                    ...
                    title=f"Evidence that disproves this paper\nPMID: {pmid}", is_answer_key=True,
                )
...
# Every evidence node the search actually found is a BLUE circle, so "evidence checked"
# reads as one consistent shape and colour that matches the legend. The known disproving
# study, when the search found it, is the same blue circle ringed in gold. The blue must
# stay clearly visible, so the dot is drawn large with a thin ring (a thick ring made the
# node read as solid yellow). A MISSED disproving study is the one exception: an orange
# triangle (a warning icon), because the recall gap is the whole point and must stand out.
_GOLD_RING = "#f08c00"
_EVIDENCE_ANSWER_KEY = {"background": "#4dabf7", "border": _GOLD_RING}
_EVIDENCE_MISSING = {"background": "#fd7e14", "border": "#d9480f"}
...
def _node_payload(node: GraphNode, control_only: bool) -> dict:
    ...
    Claim labels are drawn on the canvas always (they are the anchors); evidence labels
    ...
    if node.is_answer_key and not node.retrieved:
        # Missed disproving study: an orange triangle (a "gap" warning), a distinct shape so
        # the recall gap reads at a glance, with a dashed border to reinforce it.
        ...
        # Found disproving study: a large blue circle (so the blue stays dominant) with a thin
        # gold ring marking it as the study the search was meant to surface.
        ...
        border_width = 1.5
        shape_extra = {}
    # Recall-gap nodes carry their label from the start (the headline finding); all other
...
def _edge_payload(edge: GraphEdge) -> dict:
    ...
    if edge.kind == "retraction":
        # No on-canvas label. The dashed purple line, the legend and the hover tooltip carry
        # the meaning. An always-drawn "retracts" label stacked multiple purple texts on top of
        # each other whenever two retraction links sat close together, so it is left to hover.
        title = "This arrow runs from a retraction notice to the paper it formally withdrew."
        return {
            **base, "title": title,
            ...
            "color": {"color": _RETRACTION_COLOUR, "highlight": _RETRACTION_COLOUR,
                      "hover": _RETRACTION_COLOUR, "opacity": 0.85},
            "dashes": True, "width": 2,
        }
...
DEFAULT_SUBTITLE = (
    "A test of evidence retrieval. Each box is a medical claim, and the lines link it to the "
    "studies the search brought back, coloured by whether each study supports or disproves the "
    "claim."
)
...
HARNESS_HOW_TO_READ = (
    "What this shows: whether an automated search can find the studies that disprove a wrong "
    "medical claim. That is the one thing the whole filter depends on. Each dark blue box is a "
    "claim the medical field later overturned. Each green box is a claim that still holds "
    "today, kept as a control. For every overturned claim, our hand-checked dataset already "
    "records which study overturned it, so we know the right answer in advance. The system "
    "then searches the medical literature and we check what it brings back. A plain blue dot "
    "is a study the search found. A blue dot ringed in gold is the known disproving study, "
    "found by the search (a success). An orange triangle is that known disproving study when "
    "the search failed to find it (a miss, and the gap this test measures). The lines show how "
    "each study relates to the claim: red disproves it, green supports it, grey is neither. A "
    "language model is used in only two narrow places, to read each claim from its text and to "
    "judge whether a study disproves or supports it; it does not decide which papers are kept "
    "or dropped, and the claims and their known disproving studies here were chosen by hand."
)
...
FILTER_HOW_TO_READ = (
    ...
    "neither. Each box is a paper, coloured by the verdict that results, and a blue dot ringed "
    "in gold marks a study that disproved it. The verdict tells downstream training to keep, "
...
def _legend_html(data: GraphData) -> str:
    ...
    if any(n.group == "evidence" and n.is_answer_key and n.retrieved for n in data.nodes):
        items.append(
            ...
            f'border:3px solid {_GOLD_RING}"></span>Known disproving study, search found it</div>'
        )
    if any(n.group == "evidence" and n.is_answer_key and not n.retrieved for n in data.nodes):
        items.append(
            '<div class="legend-item"><span class="tri" style="color:#fd7e14">&#9650;</span>'
    ...
    if any(n.group == "evidence" and not n.is_answer_key for n in data.nodes):
        items.append(
            ...
            f'border-color:{_EVIDENCE_PLAIN["border"]}"></span>Other study the search checked</div>'
        )
    return "".join(items)
...
<script>
  ...
  const options = {
    ...
    physics: {
      ...
      enabled: physicsOn,
      solver: "barnesHut",
      barnesHut: { gravitationalConstant: -9000, centralGravity: 0.35, springLength: 170, springConstant: 0.045, avoidOverlap: 1, damping: 0.85 },
  ...
  // --- hover / select neighbourhood focus ---------------------------------
  ...
  function clearFocus() {
    nodes.update(rawNodes.map(n => ({ id: n.id, label: labelFor(n, false) })));
