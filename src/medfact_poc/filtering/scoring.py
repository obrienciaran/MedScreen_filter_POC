...
The score is evidence-grounded, not a model opinion: refutation strength is the stance
confidence weighted by the refuting study's evidence tier, so a high-tier RCT or guideline
that contradicts a claim moves the verdict far more than a case report. A paper is judged
as harshly as its most damning claim, because one confidently wrong central claim is
enough to want the paper down-weighted or dropped from training.

These functions are pure and unit-tested. Thresholds are module constants so the policy
is visible and tunable, not buried.
"""
...
# Default action per verdict. Unverified is kept: a missing refutation is not evidence of
# falsity. The continuous score is also written so the user can set their own threshold.
_ACTION_BY_VERDICT = {
