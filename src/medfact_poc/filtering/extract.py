...
Scoring at the claim level is more accurate than scoring a whole abstract at once,
because a paper can be right about one thing and wrong about another, and because a claim
carries conditions (population, comparator, dose, setting) that decide whether a piece of
evidence actually contradicts it.

Backends are pluggable behind ``ClaimExtractor``:
  * ``stub`` — deterministic, dependency-free. Treats the title as a single claim so the
    pipeline runs offline. Not a real extraction.
  * ``llm``  — any ``llm.LLMClient`` provider, prompted to return atomic claims as JSON.
"""
...
class LLMExtractor:
    """Claim extractor backed by any ``llm.LLMClient`` provider."""
