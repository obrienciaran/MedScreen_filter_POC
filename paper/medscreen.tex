...
The evaluation has its own gaps. The reported results were measured on a seed of twenty well-known reversals and twelve controls, the easiest cases. That set has since been doubled to sixty-four claims (twenty-eight reversals, four fabrications, and thirty-two controls), adding less-famous reversals and more ordinary controls so recall is not measured only on the most favourable examples; re-measuring on the expanded set is the immediate next step. There is still no end-to-end accuracy figure on a human-labelled set of ordinary papers, which is the most informative evaluation to add. The extraction scoring run against its reference set is written but deferred, because running the extractor needs a real LLM. The reference claims themselves were written by a strong model rather than a person, so the extraction check measures agreement between two models, not agreement with human ground truth.
